...
for stock_id in range(0, len(stock_arr)):
    ...
    base_val = profits[2]
    if profits[9] > 0:
        base_val = profits[6] + profits[7] + profits[8] + profits[9]
    elif profits[8] > 0:
        base_val = profits[6] + profits[7] + profits[8] + profits[8]
    elif profits[3] > 0:
        base_val = profits[3] # 당해년도
    elif profits[2] > 0:
        base_val = profits[2] # 전년도

    base_val = profits[2] # 19년 영업이익 기준
    ...
    col_data = (stock_type,
                ...
                multiple,
                business_category,
                stock_arr[stock_id][3])
    for idx in range(2, 10):
        val_result_ws.cell(stock_id + 2, idx, col_data[idx - 2])

    print("#" + str(stock_id) + ": " + stock_arr[stock_id][0])

val_result_wb.save("2020_4q_stock.xlsx")
print("Finished!!")
